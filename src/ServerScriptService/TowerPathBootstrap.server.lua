...
local function rebuild(tower:Folder,floorCount:number,radius:number,prefix:string)
	...
	local finish=tower:WaitForChild("TowerFinish")::BasePart;local finishPosition=point(base,radius,(floorCount-1)*ANGLE_STEP+math.rad(15),base.Y+floorCount*FLOOR_RISE+4);resizeAt(finish,Vector3.new(16,PLATFORM_HEIGHT,16),finishPosition,"Checkpoint",floorCount+1)
	local column=tower:FindFirstChild("CentralTowerColumn")or Instance.new("Part");local columnBottom=base.Y+20;local columnHeight=(floorCount+1)*FLOOR_RISE-20;column.Name="CentralTowerColumn";column.Size=Vector3.new(12,columnHeight,12);column.Position=Vector3.new(base.X,columnBottom+columnHeight/2,base.Z);column.Anchored=true;column.CanCollide=true;column.Material=Enum.Material.Brick;column.Color=Color3.fromRGB(190,125,75);column:SetAttribute("IsTowerPlatform",false);column:SetAttribute("DecorativeObject",true);column.Parent=tower
