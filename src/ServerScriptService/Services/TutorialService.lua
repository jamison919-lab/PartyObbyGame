...
function Service.Restart(p:Player)local profile=Profiles.GetProfile(p);if not profile then return end;remotes.TutorialState:FireClient(p,{completed=false,lastStep=0,steps=Config.Steps,replay=true})end
return Service
