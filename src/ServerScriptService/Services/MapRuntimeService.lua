...
local function base(id:string,count:number):Folder
	local map=Instance.new("Folder");map.Name=id;map:SetAttribute("MapId",id);map:SetAttribute("Mode","Race");map:SetAttribute("MapVersion",1);map:SetAttribute("CheckpointCount",count);map:SetAttribute("RuntimeCleanupReady",true)
	...
	folder(map,"Checkpoints");folder(map,"TrackSurfaces");folder(map,"OffTrackZones");folder(map,"ItemBoxes");folder(map,"MovingObstacles");folder(map,"Puzzles");folder(map,"RouteMarkers")
	return map
end
...
local function arrows(map:Folder,positions:{Vector3})for i,pos in positions do local arrow=part(map.RouteMarkers,"NextArrow"..i,Vector3.new(2,.4,5),CFrame.new(pos),Color3.fromRGB(255,220,65),false);arrow.Material=Enum.Material.Neon;arrow.Transparency=.15;arrow:SetAttribute("DecorativeObject",true)end end
local function clocktower():Folder
	...
	addZones(map,310);addBoxes(map,{Vector3.new(-4,7,45),Vector3.new(4,13,115),Vector3.new(-4,18,185),Vector3.new(4,24,250)});arrows(map,{Vector3.new(0,5,35),Vector3.new(0,12,105),Vector3.new(0,18,175),Vector3.new(0,24,245)});finishMap(map,last+Vector3.new(0,0,18));return map
end
local function workshop():Folder
	...
	part(puzzles,"FinalDoor",Vector3.new(26,12,2),CFrame.new(0,9,275),Color3.fromRGB(130,65,55));addZones(map,310);addBoxes(map,{Vector3.new(-6,7,35),Vector3.new(6,7,145),Vector3.new(-6,7,235)});arrows(map,{Vector3.new(0,4,30),Vector3.new(0,4,120),Vector3.new(0,4,210)});finishMap(map,Vector3.new(0,4,300));return map
end
function Service.Build(id:string):Folder
	if id=="VillageRace01"then local source=workspace:FindFirstChild("PartyObbyMap");if source and source:IsA("Folder")then local copy=source:Clone();copy.Name=id;copy:SetAttribute("MapId",id);copy:SetAttribute("Mode","Race");copy:SetAttribute("MapVersion",2);copy:SetAttribute("RuntimeCleanupReady",true);return copy end end
