...
local Service={}; local temporary=Instance.new("Folder"); temporary.Name="TemporaryPartyItems"; temporary.Parent=workspace
...
function Service.Use(p:Player):boolean
	...
	d.HeldItemId=nil; d.LastItemUseAt=os.clock(); d.ItemCooldownUntil=os.clock()+item.Cooldown; remotes.ItemUsed:FireClient(p,{id=id,name=item.DisplayName}); print(string.format("[Item] %s used %s",p.Name,id))
