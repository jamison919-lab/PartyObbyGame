...
local function visibleInHierarchy(object:GuiObject):boolean local node:Instance?=object;while node do if node:IsA("GuiObject")and not node.Visible then return false end;if node:IsA("ScreenGui")then return node.Enabled end;node=node.Parent end;return false end
task.delay(3,function()
	...
	for _,object in Players.LocalPlayer.PlayerGui:GetDescendants()do
		if object:IsA("GuiObject")and visibleInHierarchy(object)and isTopLevel(object)and object.AbsoluteSize.X>0 and object.AbsoluteSize.Y>0 then local p=object.AbsolutePosition;local s=object.AbsoluteSize;if p.X<0 or p.Y<topLeft.Y-2 or p.X+s.X>size.X-bottomRight.X+2 or p.Y+s.Y>size.Y-bottomRight.Y+2 then warnings+=1;warn("[DeviceLayout] top-level container outside safe area: "..object:GetFullName())end end
	end
