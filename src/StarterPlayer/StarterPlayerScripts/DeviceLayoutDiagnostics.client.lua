...
task.delay(3,function()
	...
	for _,object in Players.LocalPlayer.PlayerGui:GetDescendants()do
		if object:IsA("GuiObject")and object.Visible and object.AbsoluteSize.X>0 and object.AbsoluteSize.Y>0 then local p=object.AbsolutePosition;local s=object.AbsoluteSize;if p.X<0 or p.Y<topLeft.Y-2 or p.X+s.X>size.X-bottomRight.X+2 or p.Y+s.Y>size.Y-bottomRight.Y+2 then warnings+=1;if warnings<=10 then warn("[DeviceLayout] outside safe area: "..object:GetFullName())end end end
	end
