...
local function request(_,state)local gui=Players.LocalPlayer.PlayerGui:FindFirstChild("ItemGui");if not gui or not gui:IsA("ScreenGui")or not gui.Enabled then return Enum.ContextActionResult.Pass end;if state==Enum.UserInputState.Begin then use:FireServer()end;return Enum.ContextActionResult.Sink end
