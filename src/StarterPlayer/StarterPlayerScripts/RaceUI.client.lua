...
resultsUpdated.OnClientEvent:Connect(function(rows) local lines={"本局排名",""}; for _,r in rows do local extra=string.format("　+%d Coins",r.coinsEarned or 0);if r.rankChange then extra..=string.format("　RP %+d　%s → %s",r.rankChange.delta,r.rankChange.oldRank,r.rankChange.newRank)end;table.insert(lines,string.format("%d. %s — %s%s",r.rank,r.displayName,r.isFinished and string.format("%.2f 秒",r.finishTime) or "DNF",extra)) end; resultText.Text=table.concat(lines,"\n"); results.Visible=true end)
