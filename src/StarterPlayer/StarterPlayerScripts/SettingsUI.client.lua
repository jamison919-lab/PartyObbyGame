...
local close=Instance.new("TextButton");close.Name="Close";close.Size=UDim2.new(1,0,0,44);close.Text="關閉";close.Selectable=true;close.Parent=frame;close.Activated:Connect(function()gui.Enabled=false end)
...
gui:GetPropertyChangedSignal("Enabled"):Connect(function()if gui.Enabled and request and request:IsA("RemoteEvent")then request:FireServer()end end)
